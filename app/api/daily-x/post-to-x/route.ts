...
export async function POST(request: NextRequest) {
  try {
    ...
    if (imageUrls.length > 0 && !post.originalTweet?.hasVideo) {
      // Only upload images if this is not a video post
      // For video posts, the video URL is already in the text
    ...
    if (accessToken) {
      ...
      tweetResult = await response.json();
    } else {
    ...
    console.error("[PostToX] Error:", error);
    return NextResponse.json(
