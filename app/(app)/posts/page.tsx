    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-end justify-between mb-8">
        ...
        <div className="flex items-center gap-4">
          {/* バズるプロンプト作成ボタン（ポップオーバー付き） */}
          {stats.tierS >= 3 && (
            <div className="relative">
              <button
                onClick={() => buzzPrompt ? setShowBuzzModal(!showBuzzModal) : handleAnalyzeBuzz()}
              ...
              {showBuzzModal && buzzPrompt && (
                <>
                  ...
                  />
                  <div className="absolute right-0 top-full mt-2 z-50 w-[480px] bg-white rounded-2xl shadow-2xl border border-zinc-200 overflow-hidden animate-scale-in">
                    {/* Header */}
                    <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 bg-gradient-to-r from-amber-50 to-orange-50">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-zinc-900 text-sm">バズるプロンプト</span>
                        <span className="text-xs text-zinc-500">
                          ({buzzPrompt.analyzedPosts}件分析)
                        </span>
                      </div>
                    ...
                    <div className="max-h-[400px] overflow-y-auto p-4 space-y-4">
                      ...
                      <div>
                        ...
                        <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-zinc-700 whitespace-pre-wrap leading-relaxed max-h-32 overflow-y-auto">
                          {buzzPrompt.prompt}
                        </div>
                      </div>
                      ...
                      {buzzPrompt.structurePatterns && buzzPrompt.structurePatterns.length > 0 && (
                        <div>
                          ...
                          <ul className="space-y-1">
                            {buzzPrompt.structurePatterns.map((pattern, i) => (
                              <li key={i} className="flex items-start gap-2 text-xs text-zinc-600">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {buzzPrompt.samplePhrases.map((phrase, i) => (
                            <span key={i} className="px-2 py-1 bg-zinc-100 text-zinc-700 text-xs rounded-lg">
                              「{phrase}」
                            </span>
                          ))}
                        </div>
